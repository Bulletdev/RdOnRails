# BulletOnRails - Security Testing Lab
...
```bash
docker exec rdonrails-brakeman brakeman --rails7 --output /reports/brakeman-report.html --format html
```

### Semgrep
```bash
docker exec rdonrails-semgrep semgrep \
  --config=auto \
  --json \
...
### Trivy
```bash
docker exec rdonrails-trivy trivy fs \
  --format json \
...
### Nuclei
```bash
docker exec rdonrails-nuclei nuclei \
  -u http://prostaff-api:3009 \
  -json \
...
- **ProStaff API**: http://localhost:3009
...
```bash
docker ps | grep rdonrails
```

### Ver Logs
```bash
# API
docker logs rdonrails-web -f
...
```bash
docker restart rdonrails-zap
docker restart rdonrails-api
```
